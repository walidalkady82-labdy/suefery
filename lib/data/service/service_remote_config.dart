...
class ServiceRemoteConfig with LogMixin{
  ...
  ServiceRemoteConfig(this._remoteConfig);

  /// Initializes the service, sets defaults, and fetches new values.
  /// This should be called from the service locator.
  static Future<ServiceRemoteConfig> create() async {
    final remoteConfig = FirebaseRemoteConfig.instance;
    final log = Logger("RemoteConfigService");
    ...
    try {
      await remoteConfig.fetchAndActivate().withTimeout();
      log.info('Fetched & activated new config.');
    } catch (e) {
      log.warning('Failed to fetch remote config: $e');
      // If it fails, the app will just use the default values.
    }

    return ServiceRemoteConfig(remoteConfig);
  }
