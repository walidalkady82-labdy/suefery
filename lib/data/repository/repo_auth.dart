...
class RepoAuth with LogMixin implements IRepoAuth {
  ...
  static Future<RepoAuth> create({bool useEmulator = false}) async {
    ...
    final staticLogger = Logger('RepoAuth.create'); 
    final googleSignIn = GoogleSignIn.instance;
    // Use emulator only in debug mode and if requested
    if (kDebugMode && useEmulator) {
      try {
        staticLogger.info('AuthRepo: Connecting to Firebase Auth Emulator...');
        ...
        staticLogger.info('AuthRepo: Connected to Auth Emulator on localhost:9099');
      } catch (e) {
        staticLogger.severe('*** FAILED TO CONNECT TO AUTH EMULATOR: $e ***');
        staticLogger.severe('*** Make sure the emulator is running: firebase emulators:start ***');
      }
    }
    return RepoAuth._(
