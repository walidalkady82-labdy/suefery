      ...
      children: [
        ...
        if (realOrder != null) ...[
          ...
          Padding(
            padding: const EdgeInsets.only(bottom: 12.0),
            child: Text(
              '${(realOrder.progress * 100).toInt()}% ${strings.orderProgress}',
              style: theme.textTheme.bodySmall?.copyWith(color: bubbleTextColor),
            ),
          ),
        ],
        ...
        if (widget.item.isActioned || (realOrder != null && realOrder.status != OrderStatus.draft))
          // Show status if already actioned or realOrder exists and is not draft
          Text(
            widget.item.actionStatus ?? realOrder?.status.name ?? 'Actioned',
